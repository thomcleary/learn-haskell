...
firstHalf s = take (if even len then half else half + 1) s
  where
    len = length s
    half = div len 2
...
capitalize = unwords . map (\(h : t) -> toUpper h : t) . words
...
interpreter' [] x y results = results
interpreter' ("up" : commands) x y results = interpreter' commands x (y + 1) results
interpreter' ("down" : commands) x y results = interpreter' commands x (y - 1) results
interpreter' ("left" : commands) x y results = interpreter' commands (x - 1) y results
interpreter' ("right" : commands) x y results = interpreter' commands (x + 1) y results
interpreter' ("printX" : commands) x y results = interpreter' commands x y (results ++ [show x])
interpreter' ("printY" : commands) x y results = interpreter' commands x y (results ++ [show y])
interpreter' commands x y results = interpreter' commands x y results
