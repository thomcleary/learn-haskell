...
sum' i j acc
  | i == j = [next]
  | otherwise = next : sum' i (j + 1) next
  where
    next = acc + j

-- Solution
-- Can keep i within the scope of a function defined via 'where'
--
-- sums i = go 0 1
--   where go sum j
--     | j>i = []
...
mylast def xs = case xs of
  [] -> def
  [x] -> x
  (_ : tail) -> mylast def tail

-- Solution
-- Can remove a case by making the last x the default in the recursive base case
--
-- mylast def []     = def
...
indexDefault (_ : tail) i def = indexDefault tail (i - 1) def
...
sumsOf' [] prev = []
sumsOf' (head : tail) prev = next : sumsOf' tail next
  where
    next = head + prev

-- Solution
-- Can define function in 'where' to keep xs in scope
--
-- sumsOf xs = go 0 xs
...
mymaximum bigger biggest (head : tail) = mymaximum bigger next tail
  where
    next = if head `bigger` biggest then head else biggest

-- Solution
-- Can use a guard as well
--
...
-- Solution
-- Reversing the patterns makes it simpler
--
-- map2 f (a:as) (b:bs) = f a b:map2 f as bs
-- map2 f _      _      = []
