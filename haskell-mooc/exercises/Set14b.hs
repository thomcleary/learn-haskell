...
balance db account = do
  deposits <- query db balanceQuery [account] :: IO [[Int]]
  return ((sum . concat) deposits)
...
parseCommand [balance, account]
  | balance == T.pack "balance" = Just (Balance account)
parseCommand [transaction, account, amount]
  | transaction == T.pack "deposit" =
      parseInt amount
        >>= (\amount -> Just (Deposit account amount))
  | transaction == T.pack "withdraw" =
      parseInt amount
        >>= (\amount -> Just (Deposit account (negate amount)))
