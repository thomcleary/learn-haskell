...
visit maze place = do
  visited <- get
  unless (elem place visited) $ do
    put (place : visited)
    maybe (return ()) (mapM_ $ visit maze) (lookup place maze)
...
findSum2 ks ns = do
  ...
  if elem sum ns then [(first, second, sum)] else []
...
allSums :: [Int] -> [Int]
allSums xs = map sum $ mapM (\x -> [x, 0]) xs
...
instance Functor Result where
  ...
  fmap f (MkResult value) = MkResult (f value)
...
instance Functor SL where
  -- implement fmap
  fmap f (SL fa) = SL $ \state ->
    let (a, as, logs) = fa state
     in (f a, as, logs)
...
instance Monad SL where
  ...
  return x = SL (\s -> (x, s, []))
  ma >>= f = SL $ \s0 ->
    let (a, s1, log1) = runSL ma s0
        (b, s2, log2) = runSL (f a) s1
     in (b, s2, log1 ++ log2)
...
mkCounter = do
  ioRef <- newIORef 0
  return (modifyIORef ioRef (+ 1), readIORef ioRef)
